...
def generate_xlsx_report(conn, output_path, min_volume_oz=16):
    try:
        ...
        query = """
        SELECT name, volume_oz, price FROM WinesTable
        UNION ALL
        SELECT name, volume_oz, price FROM BeersTable
        UNION ALL
        SELECT name, volume_oz, price FROM IPAsTable
        UNION ALL
        SELECT name, volume_oz, price FROM BeveragesTable
        """""
